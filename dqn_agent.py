...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, fc1_units=64,
                 fc2_units=64, fc3_units=None, double_q=False):
        ...
        self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR, weight_decay=WD)
...
#             action_values = self.qnetwork_local(state)
    ...
    def learn(self, experiences, gamma):
        ...
        if self.double_q:
            am_Q_local = self.qnetwork_local(next_states).detach().max(1)[1]
            am = [list(range(am_Q_local.size()[0])), list(am_Q_local.numpy())]
            Q_targets_next = \
                 self.qnetwork_target(next_states).detach()[am].unsqueeze(1)
        else:
            Q_targets_next = \
